...
export const ProductDetails = ({ onNavigate }: { onNavigate: (view: string) => void }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!product || !contract || !account) return; // ensure wallet & contract are connected

        // Map your product data to the smart contract keys
        const keyMapping: { [K in keyof Omit<ProductData, 'batchId' | 'testDate'>]?: string } = {
            temperature: "Temperature_C",
            humidity: "Humidity_Pct",
            storageTime: "Storage_Time_Days",
            lightExposure: "Light_Exposure_hours_per_day",
            soilPh: "Soil_pH",
            soilMoisture: "Soil_Moisture_Pct",
            soilNitrogen: "Soil_Nitrogen_mgkg",
            soilPhosphorus: "Soil_Phosphorus_mgkg",
            soilPotassium: "Soil_Potassium_mgkg",
            soilCarbon: "Soil_Organic_Carbon_Pct",
            heavyMetalPb: "Heavy_Metal_Pb_ppm",
            heavyMetalAs: "Heavy_Metal_As_ppm",
            heavyMetalHg: "Heavy_Metal_Hg_ppm",
            heavyMetalCd: "Heavy_Metal_Cd_ppm",
            aflatoxinTotal: "Aflatoxin_Total_ppb",
            pesticideResidue: "Pesticide_Residue_Total_ppm",
            moistureContent: "Moisture_Content_Pct",
            essentialOil: "Essential_Oil_Pct",
            chlorophyllIndex: "Chlorophyll_Index",
            leafSpots: "Leaf_Spots_Count",
            discoloration: "Discoloration_Index",
            bacterialCount: "Total_Bacterial_Count_CFU_g",
            fungalCount: "Total_Fungal_Count_CFU_g",
            ecoliPresent: "E_coli_Present",
            salmonellaPresent: "Salmonella_Present",
            dnaAuthenticity: "DNA_Marker_Authenticity",
        };
        ...
        try {
            // ---- Step 1: Send to your analysis API ----
            setloder(true)
            ...
            if (data.status == "Anomaly Detected") {
                toast({
                    ...
                    description: `Issue: ${data.summery}. Please send this product for manual re-check.`,
                    status: "error",   // optional: makes it red if your toast library supports it
                    duration: 5000,    // visible for 5 seconds
                    isClosable: true,  // allows user to close manually
                });
                console.log(data.summery)
                setloder(false);
            }
            else {
                // ---- Step 2: Send to Ethereum contract ----
                try {
                    // Basic Info
                    ...
                    toast({
                        ...
                        description: "Laboratory results recorded and stored on blockchain.",
                    });
                    setloder(false)
                } catch (err) {
            ...
            toast({
                ...
                description: "Could not submit data. Please check the console for details.",
                variant: "destructive",
            });
        }
    };
        ...
        <div className="min-h-screen bg-background p-6">
            <div className="mb-8 max-w-4xl mx-auto">
                ...
                <div className="mb-6">
                    <h1 className="text-3xl font-bold text-gray-900">
                        Research Data Entry {product.batchId}
                    </h1>
                    <p className="text-gray-500 mt-1">
                        Enter comprehensive laboratory test results
                    </p>
                </div>
                ...
                <div className="max-w-md">
                    ...
                    {account ? (
                        <input
                            ...
                            value={account}
                            readOnly
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900"
                        />
                    ) : (
                        <button
                            onClick={conntecte}
            ...
            <div className="max-w-4xl mx-auto">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {/* Contaminants */}
                    <CollapsibleCard
                        title="Contaminants & Safety Parameters"
                        description="Heavy metals, aflatoxins, pesticide residues"
                        open={openSections.contaminants}
                        toggle={() => toggleSection("contaminants")}
                        onInputChange={handleInputChange}
                        inputs={[
                            { id: "heavyMetalPb", label: "Heavy Metal Pb (ppm)", value: product.heavyMetalPb },
                            { id: "heavyMetalAs", label: "Heavy Metal As (ppm)", value: product.heavyMetalAs },
                            { id: "heavyMetalHg", label: "Heavy Metal Hg (ppm)", value: product.heavyMetalHg },
                            { id: "heavyMetalCd", label: "Heavy Metal Cd (ppm)", value: product.heavyMetalCd },
                            { id: "aflatoxinTotal", label: "Aflatoxin Total (ppb)", value: product.aflatoxinTotal },
                            { id: "pesticideResidue", label: "Pesticide Residue Total (ppm)", value: product.pesticideResidue }
                        ]}
                    />
                    ...
                    <CollapsibleCard
                        ...
                        description="Moisture, essential oils, chlorophyll analysis"
                        ...
                        inputs={[
                            ...
                            { id: "leafSpots", label: "Leaf Spots Count", value: product.leafSpots },
                    ...
                    <CollapsibleCard
                        ...
                        description="Bacterial and fungal analysis"
                        open={openSections.microbial}
                        toggle={() => toggleSection("microbial")}
                        onInputChange={handleInputChange}
                        inputs={[
                            { id: "bacterialCount", label: "Total Bacterial Count (CFU/g)", value: product.bacterialCount },
                            { id: "fungalCount", label: "Total Fungal Count (CFU/g)", value: product.fungalCount },
                        ]}
                    >
                    ...
                    <CollapsibleCard
                        ...
                        description="DNA marker authenticity testing"
...
const InputSection = ({ id, label, value, onChange, readOnly = false }: { id: string; label: string; value: number | string; onChange?: (id: keyof ProductData, value: string) => void; readOnly?: boolean; }) => (
    <div className="space-y-2">
        ...
        <Input
            ...
            type="text"
            value={value}
            onChange={onChange ? (e) => onChange(id as keyof ProductData, e.target.value) : undefined}
            readOnly={readOnly}
        />
    </div>
);
...
const SelectSection = ({ id, label, value, onChange }: { id: string; label: string; value: string; onChange: (id: keyof ProductData, value: string) => void; }) => (
    <div className="space-y-2">
        <Label>{label}</Label>
        <Select value={value} onValueChange={(val) => onChange(id as keyof ProductData, val)}>
            <SelectTrigger>
...
const CollapsibleCard = ({ title, description, open, toggle, inputs, children, onInputChange }: any) => (
    <Card className="shadow-card">
        <Collapsible open={open} onOpenChange={toggle}>
            ...
            <CollapsibleContent>
                <CardContent className="grid grid-cols-2 lg-grid-cols-3 gap-4 pt-4">
